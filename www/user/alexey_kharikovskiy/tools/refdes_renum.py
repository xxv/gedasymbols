...
#Select replacing algorythm by proper setup coordinates location order and some computation
		...
		refdes=re.search('(.*)%s=([A-Z]*)(\d+|\?*)' %(attr) ,comp_list[i][-1],flags=re.DOTALL)
	...
	for i in comp_list: #Replacing sorted attributes
		#Complex calulate of current REFDES 
		if ref_curr_pref == i[0]:
			ref_curr_num += 1
		else:
			try:ref_curr_num = page_num + 1 if page_num != 0 else ref_last_nums[i[0]] + 1 
			except:ref_curr_num=1
			ref_curr_pref = i[0]
		ref_last_nums[ref_curr_pref]=ref_curr_num # Add current REFDES in last numbers
		i.append(str(ref_curr_num)) #Add current number to component list
		...
		content = content.replace(i[~2],i[~2].replace('%s=' %(attr) +i[0]+i[~1],'%s=' %(attr) +i[0]+i[~0])) #Replacing current string with New REFDES
		if log and verbose:
			print 'Replacing in sch : ', i[0]+i[~1], ' => ', i[0]+i[~0]
			print '<<< ', i[~2]
			print '>>> ', i[~2].replace('%s=' %(attr) +i[0]+i[~1],'%s=' %(attr) +i[0]+i[~0])
			print '------------------------------------'
		...
		if attr=='refdes':
			for pcb_index in range(0,len(pcbcontent)): #Search current REFDES in pcb files and ADD context of REFDES (appropriate element ontex completely) to list of replaces
				elem = re.search('Element\[[^[\]]*"%s"[^[\]]*\]' %(i[0]+i[~1]),pcbcontent[pcb_index] ,flags=re.DOTALL)
				if elem:
					try:
						pcb_replaces.append([pcb_index,elem.group(),elem.group().replace(i[0]+i[~1],i[0]+i[~0])]) 
					except:pcb_replaces =  [[pcb_index,elem.group(),elem.group().replace(i[0]+i[~1],i[0]+i[~0])]] #Create new replace list
					if log and verbose:
						print 'Replacing in pcb: ', i[0]+i[~1], ' => ', i[0]+i[~0]
						print '<<< ', elem.group()
						print '>>> ', elem.group().replace(i[0]+i[~1],i[0]+i[~0])
						print '------------------------------------'
